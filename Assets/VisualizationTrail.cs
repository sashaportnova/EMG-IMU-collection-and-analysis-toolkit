...
public class VisualizationTrail : MonoBehaviour
{
    ...
    void ShiftTrailLeft()
    {
        ...
        for (int i = 0; i < trailLength; i++)
        {
            RectTransform point = trailPoints[i];
            Vector2 pos = point.anchoredPosition;

            if (pos.x < leftBoundary - 50f) continue; // Give buffer zone and skip very offscreen

            float newX = pos.x - shiftAmount;
            if (Mathf.Abs(newX - pos.x) < 0.01f) continue; // Skip minimal movement
            ...
            if (image != null)
            {
                ...
                {
                    image.enabled = false; // Disable instead of moving offscreen
                }
                else
                {
